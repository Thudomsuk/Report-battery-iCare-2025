...
import { exportToCSV, exportToPNG, formatBranchDataForCSV } from '../../utils/exportUtils';
...
export const BranchSummary: React.FC<BranchSummaryProps> = ({ data, isLoading }) => {
  if (isLoading) {
    return (
      <div className="flex justify-center items-center py-12">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600"></div>
        <span className="ml-3 text-gray-600">กำลังโหลดข้อมูลสาขา...</span>
      </div>
    );
  }
  ...
  if (!data || data.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500">ไม่มีข้อมูลการจองในขณะนี้</p>
      </div>
    );
  }
  ...
  const handleExportCSV = () => {
    const csvData = formatBranchDataForCSV(data);
    exportToCSV(csvData, `สรุปการจองตามสาขา_${new Date().toLocaleDateString('th-TH')}`);
  };

  const handleExportPNG = () => {
    exportToPNG('branch-summary-table', `สรุปการจองตามสาขา_${new Date().toLocaleDateString('th-TH')}`);
  };

  return (
    <div id="branch-summary-table" className="bg-white rounded-lg shadow-lg overflow-hidden">
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-6">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-bold mb-2">สรุปการจองตามสาขา</h2>
            <p className="text-blue-100">ยอดจองรวม: {totalBookings.toLocaleString()} ครั้ง</p>
          </div>
          <ExportButtons 
      ...
      <div className="bg-gray-50 px-6 py-4">
        <div className="flex justify-between items-center text-sm text-gray-600">
          ...
          <span>อัพเดทข้อมูลทุก 5 นาที</span>
        </div>
      </div>
    </div>
  );
