...
interface DailyBranchSummaryProps {
  data: DailyBookingData[];
  isLoading?: boolean;
}

export const DailyBranchSummary: React.FC<DailyBranchSummaryProps> = ({ data, isLoading }) => {
  ...
  const handleExportCSV = () => {
    ...
    const filename = selectedDay === 'all' 
      ? `สรุปการจองรายวัน_${new Date().toLocaleDateString('th-TH')}`
      : `สรุปการจอง_${getDayLabel(selectedDay)}_${new Date().toLocaleDateString('th-TH')}`;
  ...
  const handleExportPNG = () => {
    ...
    exportToPNG('daily-branch-summary-table', filename);
  };
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center py-12">
        ...
        <span className="ml-3 text-gray-600">กำลังโหลดข้อมูลรายวัน...</span>
      </div>
    );
  }
  ...
  if (!data || data.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500">ไม่มีข้อมูลการจองรายวันในขณะนี้</p>
      </div>
    );
  }

  return (
    <div id="daily-branch-summary-table" className="bg-white rounded-lg shadow-lg overflow-hidden">
      <div className="bg-gradient-to-r from-indigo-600 to-blue-600 text-white p-6">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-bold mb-2">สรุปการจองรายวัน</h2>
            <p className="text-indigo-100">
              {selectedDay === 'all' 
                ? `ยอดจองรวม: ${totals.total.toLocaleString()} ครั้ง`
                : `ยอดจอง${getDayLabel(selectedDay)}: ${totals[selectedDay as keyof typeof totals].toLocaleString()} ครั้ง`
              }
            </p>
          </div>
            ...
            <tr className="bg-gradient-to-r from-blue-100 to-indigo-100 border-t-2 border-blue-300">
              ...
              {selectedDay === 'all' ? (
                <>
                  ...
                  <td className="px-6 py-4 text-center">
                    <span className="text-xl font-bold text-blue-900 bg-blue-200 px-3 py-1 rounded">
